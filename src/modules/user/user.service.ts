...
export class UserService {
  ...
  public async registerUser(
    ...
    // 查询unionid存在的用户
    const currentUser = await this.userRepository.findOneBy({ openid });
    // console.log('currentUser', currentUser)
    if (currentUser)
      throw new FailException(
        ERROR_CODE.USER.USER_UNION_ID_EXISTS,
        RESPONSE_DEFAULT_TEXT.ACCOUNT_EXISTS
      );
  ...
  public async login(loginInfo: LoginCodeDto): Promise<IUserLoginResponse> {
    ...
    const { APPID, SECRET, GRANT_TYPE } = WECHAT_MINI_PRO;
    const checkResultObservable = this.http.get(WECHAT_CODE_SWTICH_ID_URL, {
      params: {
        appid: APPID,
        secret: SECRET,
        js_code: code,
    ...
    if (status !== USER_STATUS.NORMAL) {
      await this.userRepository.update({ openid }, { status: 1 });
    }
  ...
  public async cancelUser(id: number): Promise<void> {
    ...
    if (!userProfile)
      throw new FailException(ERROR_CODE.USER.USER_PROFILE_ERROR);
    // 删除redis中的缓存
    if (userProfile.id) {
   ...
   * @param longitude
   */
  public async queryCityApi(
    id: number,
    LocationAxis: { latitude: string; longitude: string }
  ): Promise<{ any }> {
    const { latitude, longitude } = LocationAxis;

    const checkResultObservable = this.http.get(
      `${WECHAT_LOCATION_URL}?location=${latitude},${longitude}&key=${TENCENT_MAP_LOCATION_KEY}`
    );
    const res = await lastValueFrom(checkResultObservable);
